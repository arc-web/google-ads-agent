...
def service_match_score(service: str, page: dict[str, Any]) -> int:
    ...
    if normalize(service) in text:
        score += 6
    if service_tokens & RESTAURANT_SERVICE_TERMS:
        score += sum(1 for token in RESTAURANT_EVIDENCE_TERMS if token in text)
    return score
...
def infer_buyer_type(service: str, evidence_text: str) -> str:
    ...
    restaurant_score = sum(1 for term in RESTAURANT_EVIDENCE_TERMS if term in lower)
    b2b_score = sum(
        1
        for term in (
            "capacity",
...
def logic_for_service(service: str, matched_pages: list[dict[str, Any]]) -> ServiceLogic:
    ...
    if "lay counselor" in service_lower:
    ...
    elif set(tokens(service_lower)) & RESTAURANT_SERVICE_TERMS:
        buyer_type = "b2c"
        ...
        mechanism = "Restaurant reservation for a tasting menu experience"
