...
def service_description_specs(
    *,
    ...
    return [
        (
            append_cta(
                f"{buyer.title()} compare {mechanism_short} for {outcome_short} with local team support",
                ctas[0],
            ),
            "service_logic",
        ),
        (
            append_cta(
                f"{buyer.title()} review {mechanism_short} for {outcome_short} with practical guidance",
                ctas[1 % len(ctas)],
            ),
            "service_logic",
        ),
        (
            append_cta(
                f"{buyer.title()} plan {mechanism_short} for {outcome_short} with experienced support",
                ctas[2 % len(ctas)],
            ),
            "service_logic",
        ),
        (
            append_cta(
                insurance_language or f"{buyer.title()} compare trusted {mechanism_short}{delivery_copy} for {outcome_short}",
                ctas[3 % len(ctas)],
            ),
            "differentiator",
        ),
...
def service_logic_buyer_term(service_logic: Any) -> str:
    ...
    if "employer" in buyer:
        return "employers"
    if "clinical" in buyer or "team" in buyer:
        return "teams"
    if "guest" in buyer:
        return "guests"
    return "customers"
