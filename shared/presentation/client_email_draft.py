...
def build_client_email_draft(data: EmailDraftInput) -> str:
    ...
    confirmation_items = data.confirmation_items or [
    ...
    if data.search_term_question_groups:
        lines.extend(
            [
                "",
                "**Search Term Questions**",
                "We grouped these so you only need to confirm items that are not already clear from the website, campaign structure, or approved service area.",
            ]
        )
        for section_title, group_type in (
            ("**Service Confirmations**", "service"),
            ("**Regional Confirmations**", "regional"),
            ("**Exclude Recommendations**", "exclude"),
        ):
            section_groups = [group for group in data.search_term_question_groups if group.group_type == group_type]
